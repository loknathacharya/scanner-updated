...
class BacktestOptimizationResponse(BaseModel):
    """Response model for parameter optimization results"""
    best_params: Dict[str, Any]
    best_performance: Dict[str, Any]
    all_results: List[Dict[str, Any]]
    execution_time: float
    signals_processed: int
    # Optional warnings/info from risk manager
    ...
    async def run_backtest(self, request: BacktestRequest, use_cache: bool = True,
        ...
        try:
            ...
            if trades_df is not None and not trades_df.empty and calc_metrics is not None:
                ...
                # Convert numpy types to Python native types for JSON serialization
                performance_metrics = convert_numpy_types({k: v for k, v in metrics.items() if k != "Equity Curve"})
                ...
                if isinstance(eq, pd.DataFrame):
                    ...
                    equity_curve_records = convert_numpy_types([{str(k): v for k, v in rec.items()} for rec in eq_records])
            ...
            if trades_df is not None and not trades_df.empty:
                ...
                trades_list = convert_numpy_types(td.to_dict('records'))
            ...
            logger.info(f"[Backtest] Completed: {execution_time:.2f}s, signals used: {len(signals_df)}, trades: {len(trades_list)}")
...
async def optimize_backtest_parameters(
    ...
    """
    Run parameter optimization for backtest using vectorized engine functions.
    ...
    try:
        ...
        # Use the BacktestOptimizer which delegates to the vectorized engine function map
        ...
        logger.info(f"Parameter optimization completed: {execution_time:.2f}s, {len(signals_df)} signals processed")
...
def _generate_param_combinations(param_ranges: Dict[str, List[float]]) -> List[Dict[str, float]]:
